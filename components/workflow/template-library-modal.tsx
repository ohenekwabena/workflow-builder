    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          className="w-full max-w-5xl max-h-[85vh] bg-white dark:bg-gray-900 rounded-2xl shadow-2xl overflow-hidden mx-4"
          ...
          <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500 mt-1">Start with a pre-built workflow</p>
              </div>
            </div>
            <button
              onClick={onClose}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {filteredTemplates.map((template) => (
                  <motion.div
                    ...
                    whileTap={{ scale: 0.98 }}
                    className="border-2 border-gray-200 dark:border-gray-700 rounded-xl p-5 hover:border-purple-500 dark:hover:border-purple-500 transition-all cursor-pointer group"
                    onClick={() => handleSelectTemplate(template)}
                  >
                    <div className="flex items-start gap-4">
                      <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-purple-500 to-pink-500 flex items-center justify-center text-3xl shrink-0 group-hover:scale-110 transition-transform">
                        {template.icon}
                      </div>
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="mt-3 text-xs text-gray-400">{template.nodes.length} nodes</div>
                      </div>
                    </div>
                  </motion.div>
                ))}
